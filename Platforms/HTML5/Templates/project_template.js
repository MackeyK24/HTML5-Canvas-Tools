...
if (%SESSION_STORAGE_COMMAND_LINE_KEY%) {
	var sessionStorageCommandLine = window.sessionStorage.getItem(%SESSION_STORAGE_COMMAND_LINE_KEY%);
	//if (sessionStorageCommandLine) {
	//	console.log("sessionStorageCommandLine: " + sessionStorageCommandLine);
	//}
}

Module.arguments = sessionStorageCommandLine != null ? [sessionStorageCommandLine] : [%UE4CMDLINE%];
...
if (%SESSION_STORAGE_WEB_SOCKET_URL_KEY%) {
	var sessionStorageWebSocketUrl = window.sessionStorage.getItem(%SESSION_STORAGE_WEB_SOCKET_URL_KEY%);
...
function addLog(info, color) {
	// Mackey Kinard: $("#logwindow").append("<h4><small>" + info + " </small></h4>");
	console.log(info);
}
Module.print = addLog;
...
var TASK_SHADERS = 2;
var TASK_MAIN = 3;
var loadTasks = [ 'Preparing Assets', 'Compiling Assemblies', 'Building Shader Graph', 'Starting Unreal Engine'];

/* DEPRECATED: Mackey Kinard
...
function taskProgress(taskId, progress) {
	...
	var info = 'Loading';
	if (progress && progress.total) {
		// DEPRECATED: text += (' ' + (progress.currentShow || progress.current) + '/' + (progress.totalShow || progress.total) + ' (' + (progress.current * 100 / progress.total).toFixed(0) + '%)');
		// DEPRECATED: text += (' ' + (progress.currentShow || progress.current) + ' / ' + (progress.totalShow || progress.total));
		text = ('Downloading ' + (progress.currentShow || progress.current) + ' / ' + (progress.totalShow || progress.total));
		if (progress.total > 0) {
			var percent = parseInt((progress.current * 100 / progress.total).toFixed(0));
			if (percent < 100) info = percent.toString() + '%';
		}
	} else {
		if (taskId === 0) {
			info = 'Loading';
	...
	if (status) status.innerHTML = info;
	var details = document.getElementById('details');
	if (details) details.innerHTML = text;
}
...
// ----------------------------------------
// ----------------------------------------
// Startup task which is run after UE4 engine has launched (Mackey Kinard)
...
//	canvasAspectRatioHeight = UE_JSlib.UE_GSystemResolution_ResY();
	...
	setTimeout(finishRunEmscripten, %PREWARMTIME%);
}
function finishRunEmscripten() {
	$("#compilingmessage").remove();

	Module['canvas'].style.display = 'block';
	document.getElementById('buttonarea').style.display = '%DEVTOOLBARDISPLAY%';
	document.getElementById('fullscreenbutton').style.display = '%FULLSCREENDISPLAY%';

	window.focus(); // The following is needed if game is within an iframe - main window already has focus...
}
